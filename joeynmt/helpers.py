...
def log_data_info(train_data: Dataset, valid_data: Dataset, test_data: Dataset,
    ...
    logging_function("First training example:\n\t[SRC_TCP] %s\n\t[SRC_TSL] %s\n\t[TRG] %s",
        " ".join(vars(train_data[0])['src_tcp']),
        " ".join(vars(train_data[0])['src_tsl']),
...
def bpe_postprocess(string) -> str:
    ...
    This is for sentencebpe, the original one is for subword-nmt
...
def bpe_postprocess_sp(string) -> str:
    """
    Post-processor for BPE output. Recombines BPE-split tokens.
