...
from joeynmt.helpers import bpe_postprocess, bpe_postprocess_sp, load_config, make_logger,\
    get_latest_checkpoint, load_checkpoint, store_attention_plots
from joeynmt.metrics import bleu, chrf, token_accuracy, sequence_accuracy
from joeynmt.multisource_model import build_model, Model
from joeynmt.multisource_batch import Batch
from joeynmt.multisource_data import load_data, make_data_iter, MonoDataset
...
def validate_on_data(model: Model, data: Dataset,
                     ...
                     beam_size: int = 3, beam_alpha: int = -1,
    ...
    valid_sources_tcp_raw = data.src_tcp
    valid_sources_tsl_raw = data.src_tsl
    ...
    with torch.no_grad():
        ...
        valid_sources_tcp = [join_char.join(s) for s in data.src_tcp]
        valid_sources_tsl = [join_char.join(s) for s in data.src_tsl]
        valid_references = [join_char.join(t) for t in data.trg]
        valid_hypotheses = [join_char.join(t) for t in decoded_valid]

        # post-process
        if level == "bpe":
            valid_sources_tcp = [bpe_postprocess(s) for s in valid_sources_tcp]
            valid_sources_tsl = [bpe_postprocess(s) for s in valid_sources_tsl]
    ...
    return current_valid_score, valid_loss, valid_ppl, valid_sources_tcp, valid_sources_tsl, \
...
def test(cfg_file,
    ...
    for data_set_name, data_set in data_to_predict.items():
        ...
        score, loss, ppl, sources_tcp, sources_tsl, sources_tcp_raw,sources_tsl_raw, references, hypotheses, \
