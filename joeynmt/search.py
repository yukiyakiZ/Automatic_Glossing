...
from joeynmt.multisource_decoders import Decoder, TransformerDecoder
from joeynmt.embeddings import Embeddings
from joeynmt.helpers import tile


__all__ = ["greedy", "transformer_greedy", "beam_search", "multisource_greedy","multisource_beam_search", "multisource_beam_search_for_gloss"]
...
def transformer_greedy(
        src_tcp_mask: Tensor,src_tsl_mask: Tensor, embed: Embeddings,
        bos_index: int, eos_index: int,
        max_output_length: int, decoder: Decoder,
        encoder_tcp_output: Tensor, encoder_tsl_output: Tensor, 
        encoder_tcp_hidden: Tensor, encoder_tsl_hidden: Tensor) -> (np.array, None):
    """
    ...
    batch_size = src_tcp_mask.size(0)

    # start with BOS-symbol for each sentence in the batch
    ys_tcp = encoder_tcp_output.new_full([batch_size, 1], bos_index, dtype=torch.long)
    ys_tsl = encoder_tsl_output.new_full([batch_size, 1], bos_index, dtype=torch.long)
    ys = ys_tcp + ys_tsl
    # a subsequent mask is intersected with this in decoder forward pass
    trg_mask = src_tcp_mask.new_ones([1, 1, 1])

    finished = src_tcp_mask.new_zeros((batch_size)).byte()
    ...
    for _ in range(max_output_length):
        ...
        with torch.no_grad():
            logits, out, _, _ = decoder(
                ...
                encoder_tcp_hidden=None,
                encoder_tsl_hidden=None,
...
def multisource_greedy(
        ...
        encoder_tcp_hidden: Tensor, encoder_tsl_hidden: Tensor) -> (np.array, None):
    """
    Special greedy function for multisource_transformer, since it works differently.
    The transformer remembers all previous states and attends to them.
...
# pylint: disable=too-many-statements,too-many-branches
def multisource_beam_search(
        decoder: Decoder,
        size: int,
        bos_index: int, eos_index: int, pad_index: int,
        encoder_tcp_output: Tensor, encoder_tsl_output: Tensor,
        encoder_tcp_hidden: Tensor, encoder_tsl_hidden: Tensor,
        src_tcp_mask: Tensor, src_tsl_mask: Tensor, max_output_length: int, alpha: float,
        embed: Embeddings, n_best: int = 1) -> (np.array, np.array):
    """
    Beam search with size k.
    Inspired by OpenNMT-py, adapted for Transformer.
    ...
    :param eos_index:
    :param pad_index:
    :param encoder_tcp_output:
    :param encoder_tsl_output:
    :param encoder_tcp_hidden:
    :param encoder_tsl_hidden:
    :param src_tcp_mask:
    :param src_tsl_mask:
    :param max_output_length:
    :param alpha: `alpha` factor for length penalty
    ...
    # Recurrent models only: initialize RNN hidden state
    # pylint: disable=protected-access
    # not considering multisource rnn here
    if not transformer:
        hidden = decoder._init_hidden(encoder_hidden)
    else:
        hidden = None
    ...
    encoder_tcp_output = tile(encoder_tcp_output.contiguous(), size,
                          dim=0)  # batch*k x src_len x enc_hidden_size
    encoder_tsl_output = tile(encoder_tsl_output.contiguous(), size,
                          dim=0)  # batch*k x src_len x enc_hidden_size
    src_tcp_mask = tile(src_tcp_mask, size, dim=0)  # batch*k x 1 x src_len
    src_tsl_mask = tile(src_tsl_mask, size, dim=0)  # batch*k x 1 x src_len

    # Transformer only: create target mask
    if transformer:
        trg_mask = src_tcp_mask.new_ones([1, 1, 1])  # transformer only
    else:
        trg_mask = None

    # numbering elements in the batch
    batch_offset = torch.arange(
        batch_size, dtype=torch.long, device=encoder_tcp_output.device)
    ...
    beam_offset = torch.arange(
        ...
        step=size,
        dtype=torch.long,
        device=encoder_tcp_output.device)
    ...
    # Give full probability to the first beam on the first step.
    topk_log_probs = torch.zeros(batch_size, size, device=encoder_tcp_output.device)
    topk_log_probs[:, 1:] = float("-inf")
    ...
    for step in range(max_output_length):
        ...
        # pylint: disable=unused-variable
        trg_embed = embed(decoder_input)
        # use the multisource decoder which receive output of two encoders and add up the attention
        logits, hidden, att_scores, att_vectors = decoder(
            encoder_tsl_output=encoder_tsl_output,
            encoder_tcp_output=encoder_tcp_output,
            ...
            trg_embed=trg_embed,
            hidden=hidden,
            prev_att_vector=att_vectors,  # not sure about this, previously it is said that att_vectors is not used for Transformer
            unroll_steps=1,
            trg_mask=trg_mask  # subsequent mask for Transformer only
        )
        # print('logits')
        ...
        # add length constriction later (i.e. len(tcp)==len(tgt))
        ...
        # reorder indices, outputs and masks
        select_indices = batch_index.view(-1)
        encoder_tcp_output = encoder_tcp_output.index_select(0, select_indices)
        encoder_tsl_output = encoder_tsl_output.index_select(0, select_indices)
        src_tcp_mask = src_tcp_mask.index_select(0, select_indices)
        src_tsl_mask = src_tsl_mask.index_select(0, select_indices)
...
# pylint: disable=too-many-statements,too-many-branches
def multisource_beam_search_for_gloss(
        decoder: Decoder,
        size: int,
        bos_index: int, eos_index: int, pad_index: int,
        encoder_tcp_output: Tensor, encoder_tsl_output: Tensor,
    ...
    results = {
        ...
        "scores": [[] for _ in range(batch_size)],
        "gold_score": [0] * batch_size,
        "attention" : [[] for _ in range(batch_size)], # store attention weitght
    }
