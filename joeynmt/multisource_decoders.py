...
class RecurrentDecoder(Decoder):
    ...
    def __init__(self,
                 ...
                 emb_size: int = 0,
                 hidden_size: int = 0,
                 encoder_tcp: Encoder = None,
                 encoder_tsl: Encoder = None,
    ...
    def _forward_step(self,
        ...
        context_tcp, att_probs_tcp = self.attention(
            query=query, values=encoder_tcp_output, mask=src_tcp_mask)
        context_tsl, att_probs_tsl = self.attention(
            query=query, values=encoder_tsl_output, mask=src_tsl_mask)

        context = context_tcp + context_tsl
    ...
    def forward(self,
                trg_embed: Tensor,
                encoder_tsl_output: Tensor,
                encoder_tcp_output: Tensor,
                encoder_hidden_tsl: Tensor,
                encoder_hidden_tcp: Tensor,
                src_tsl_mask: Tensor,
                src_tcp_mask: Tensor,
                unroll_steps: int,
                hidden: Tensor = None,
        ...
        self._check_shapes_input_forward(
            ...
            encoder_hidden_tsl=encoder_hidden_tsl,
            encoder_hidden_tcp=encoder_hidden_tcp,
...
class TransformerDecoder(Decoder):
    ...
    def __init__(self,
        ...
        self.layers = nn.ModuleList([MultisourceTransformerDecoderLayer(
    ...
    def forward(self,
                ...
                encoder_tcp_output: Tensor = None,
                encoder_tsl_output: Tensor = None,
                encoder_hidden_tcp: Tensor = None,
                encoder_hidden_tsl: Tensor = None,
                src_tcp_mask: Tensor = None,
                src_tsl_mask: Tensor = None,
        ...
        for layer in self.layers:
            x = layer(x=x, memory_tcp=encoder_tcp_output, memory_tsl=encoder_tsl_output,
                      src_tcp_mask=src_tcp_mask, src_tsl_mask=src_tsl_mask, trg_mask=trg_mask)
        x = self.layer_norm(x)
