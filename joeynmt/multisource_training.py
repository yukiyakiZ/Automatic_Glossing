...
from joeynmt.multisource_model import build_model
from joeynmt.multisource_batch import Batch
from joeynmt.helpers import log_data_info, load_config, log_cfg, \
    store_attention_plots, load_checkpoint, make_model_dir, \
    make_logger, set_seed, symlink_update, ConfigurationError
from joeynmt.multisource_model import Model
from joeynmt.prediction import validate_on_data
from joeynmt.loss import XentLoss
from joeynmt.multisource_data import load_data, make_data_iter
...
class TrainManager:
    ...
    def __init__(self, model: Model, config: dict) -> None:
        ...
        self.scheduler, self.scheduler_step_at = build_scheduler(
            ...
            hidden_size=config["model"]["encoder_tcp"]["hidden_size"])
    ...
    def train_and_validate(self, train_data: Dataset, valid_data: Dataset) \
        ...
        for epoch_no in range(self.epochs):
            ...
            for i, batch in enumerate(iter(train_iter)):
                ...
                if self.steps % self.validation_freq == 0 and update:
                    ...
                    valid_score, valid_loss, valid_ppl, valid_sources_tcp, valid_sources_tsl, \
                        valid_sources_tcp_raw,valid_sources_tsl_raw, valid_references, valid_hypotheses, \
                        ...
                        validate_on_data(
                            ...
                            beam_size=3,  # beam search validations
                    ...
                    self._log_examples(
                        sources_tcp_raw=[v for v in valid_sources_tcp_raw],
                        sources_tsl_raw=[v for v in valid_sources_tsl_raw],
                        sources_tcp=valid_sources_tcp,
                        sources_tsl=valid_sources_tsl,
        ...
        self.logger.info('Best validation result (beam_size=3) at step '
    ...
    def _log_examples(self, sources_tcp: List[str], sources_tsl: List[str], hypotheses: List[str],
                      references: List[str],
                      sources_tcp_raw: List[List[str]] = None,
                      sources_tsl_raw: List[List[str]] = None,
        ...
        for p in self.log_valid_sents:
            ...
            if p >= len(sources_tcp):
                continue

            self.logger.info("Example #%d", p)

            if sources_tcp_raw is not None:
                self.logger.debug("\tRaw source for transcription:     %s", sources_tcp_raw[p])
            if sources_tsl_raw is not None:
                self.logger.debug("\tRaw source for translation:     %s", sources_tsl_raw[p])
            ...
            self.logger.info("\tSource transcription:     %s", sources_tcp[p])
            self.logger.info("\tSource translation:     %s", sources_tsl[p])
